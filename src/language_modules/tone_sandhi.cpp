...
namespace melo {
    namespace ToneSandhi {
         ...
         * @param sub_finals: 韵母
         */
        void modified_tone(const std::string& word, const std::string& tag, const std::shared_ptr<cppjieba::Jieba>& jieba, std::vector<std::string>& sub_finals) {
        ...
        void _yi_sandhi(const std::vector<std::string>& chinese_characters, std::vector<std::string>& sub_finals) {
            ...
            if (chinese_characters.size() == 3 && chinese_characters[1] ==  "一" && chinese_characters[0] == chinese_characters[2]) {
                sub_finals[1].back() = '5';
                return;
            }
        ...
        void _neural_sandhi(const std::string& word, const std::vector<std::string>& chinese_characters, const std::string& pos, const std::shared_ptr<cppjieba::Jieba>& jieba, std::vector<std::string>& sub_finals) {
            ...
            for (size_t i = 1; i <n; i++) {
                auto& c = chinese_characters[i];
                if (chinese_characters[i] == chinese_characters[i - 1] && (
                    pos[0] == 'n' || pos[0] == 'v' || pos[0] == 'a'))
                {
                    sub_finals[i].back() = '5';
                }
            }
            const static std::unordered_set<std::string> nenuralCharSet = {
                "吧", "呢", "啊", "呐", "噻", "嘛",
                "吖", "嗨", "呐", "哦", "哒", "额",
                "滴", "哩", "哟", "喽", "啰", "耶",
                "喔", "诶", "的", "地", "得"
            ...
            if (nenuralCharSet.contains(chinese_characters.back()))
            {
                ...
                (chinese_characters.back() == "们" || chinese_characters.back() =="字")
                && (pos[0]=='n' || pos[0] == 'r')) {
                sub_finals.back().back() = '5';
            }
            else if
                ((chinese_characters.back() == "上" || chinese_characters.back() == "下" || chinese_characters.back() == "里") //e.g. 桌上, 地下, 家里
                && (pos[0] == 's' || pos[0] == 'l' || pos[0] == 'f'))
            {
                sub_finals.back().back() = '5';
            }
            else if 
            ((chinese_characters.back() == "来" || chinese_characters.back() == "去") && st1.contains(chinese_characters[n-2])) { //e.g. 上来, 下去
                sub_finals.back().back() = '5';
            }
            auto it = std::find(chinese_characters.begin(), chinese_characters.end(), "个");
            // "个"做量词
            if (it != chinese_characters.end() && it != chinese_characters.begin() && (is_numeric(*(it-1)) || st2.contains(*(it - 1))))
            {
        ...
        void _three_sandhi(const std::string& word, const std::vector<std::string>& chinese_characters, const std::shared_ptr<cppjieba::Jieba>& jieba, std::vector<std::string>& sub_finals) {
